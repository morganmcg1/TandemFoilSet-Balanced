...
- **Timeout**: each training execution is capped by `SENPAI_TIMEOUT_MINUTES` (wall clock) and `--epochs`. These are hard upper bounds, not target durations; choose debug, screening, or longer confirmation runs according to the evidence needed. Do not override these limits.
