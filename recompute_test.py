"""One-shot recomputation of test metrics with NaN-safe accumulation.

Pre-existing bug in data/scoring.py (read-only): `accumulate_batch` filters
samples whose y is non-finite by setting `surf_mask`/`vol_mask` to False on
those rows, but the error tensor still contains NaN at those positions, and
`NaN * 0 = NaN`, so the NaN propagates into the accumulator.

This script re-evaluates the saved best checkpoint on the four test splits
using a local NaN-safe accumulator (without touching the read-only file) so
we can report a numeric test_avg/mae_surf_p for the experiment.
"""
...
from data.scoring import finalize_split

ACTIVATION = {"gelu": nn.GELU, "silu": nn.SiLU}
...
class Transolver(nn.Module):
    def __init__(self, space_dim=1, n_layers=5, n_hidden=256, dropout=0.0, n_head=8, act="gelu",
...
def safe_accumulate(pred_orig, y, is_surface, mask, mae_surf, mae_vol):
    """NaN-safe accumulator. Mirrors data/scoring.py semantics (skip samples with
    any non-finite y) but zeroes the error tensor BEFORE the masked sum so that
    masked-out positions don't poison the accumulator with NaN."""
    B = y.shape[0]
...
def evaluate_split_safe(model, loader, stats, surf_weight, device):
    ...
    with torch.no_grad():
        for x, y, is_surface, mask in loader:
            ...
            ds_, dv_ = safe_accumulate(pred_orig, y, is_surface, mask, mae_surf, mae_vol)
            n_surf += ds_
            n_vol += dv_
...
if __name__ == "__main__":
    ...
    print("Re-evaluating test splits with NaN-safe scoring...")
    test_metrics = {
        name: evaluate_split_safe(model, loader, stats, 10.0, device)
        for name, loader in test_loaders.items()
    }
    test_avg = aggregate_splits(test_metrics)

    out_path = model_dir / "test_recomputed.json"
    payload = {"test_avg": test_avg, "test_splits": test_metrics}
    ...
    for name in TEST_SPLIT_NAMES:
        m = test_metrics[name]
        print(
            f"  {name:<26s} surf[p={m['mae_surf_p']:.4f} "
            f"Ux={m['mae_surf_Ux']:.4f} Uy={m['mae_surf_Uy']:.4f}]  "
            f"vol[p={m['mae_vol_p']:.4f} "
