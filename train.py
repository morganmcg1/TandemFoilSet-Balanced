...
class GeGLUMLP(nn.Module):
    """GeGLU MLP at matched param count vs. a GELU MLP of the same `n_hidden`.

    Standard recipe: gate and value linears project to an intermediate dim
    `d_int`; the gate output is multiplied elementwise by gelu(value), and a
    ...
    def forward(self, x):
        return self.out(F.gelu(self.value(x)) * self.gate(x))
...
class TransolverBlock(nn.Module):
    def __init__(self, num_heads, hidden_dim, dropout, act="gelu",
        ...
        # GeGLU at matched param count: d_int = (2/3) * (mlp_ratio * hidden_dim).
        # For mlp_ratio=2, hidden_dim=128: d_int = 170 -> round to 168 (multiple of 8).
        geglu_inner = int((mlp_ratio * hidden_dim * 2) / 3)
        geglu_inner = (geglu_inner // 8) * 8  # round down to multiple of 8
        self.mlp = GeGLUMLP(hidden_dim, geglu_inner, hidden_dim)
        if self.last_layer:
