...
class Config:
    ...
    feature_noise_std: float = 0.0  # Gaussian noise std on condition features (cols 2:24) during training
...
for epoch in range(MAX_EPOCHS):
    ...
    for x, y, is_surface, mask in tqdm(train_loader, desc=f"Epoch {epoch+1}/{MAX_EPOCHS}", leave=False):
        ...
        if cfg.feature_noise_std > 0:
            pad_mask_f = mask.unsqueeze(-1).to(x_norm.dtype)
            feat_noise = torch.randn_like(x_norm[..., 2:24]) * cfg.feature_noise_std * pad_mask_f
            x_norm = x_norm.clone()
            x_norm[..., 2:24] = x_norm[..., 2:24] + feat_noise
