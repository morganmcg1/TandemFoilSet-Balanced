...
def evaluate_split(model, loader, stats, surf_weight, device,
                   bf16: bool = False) -> dict[str, float]:
...
class Config:
    ...
    loss: str = "l1"              # "mse" | "l1"
    bf16: bool = True             # torch.autocast("cuda", dtype=torch.bfloat16)
...
def update_ema(src, dst, decay: float) -> None:
    with torch.no_grad():
        for ema_p, p in zip(dst.parameters(), src.parameters()):
            ema_p.mul_(decay).add_(p, alpha=1 - decay)
        for ema_b, b in zip(dst.buffers(), src.buffers()):
            ema_b.copy_(b)
...
if best_metrics:
    ...
    eval_model_for_test.load_state_dict(
