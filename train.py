...
class DropPath(nn.Module):
    """Stochastic depth (per-sample) — drops the whole residual branch with
    probability ``drop_prob`` during training. Inference is unchanged.
    """
...
class Transolver(nn.Module):
    def __init__(self, space_dim=1, n_layers=5, n_hidden=256, dropout=0.0,
        ...
        dpr = [x.item() for x in torch.linspace(0.0, drop_path_max, n_layers)]
        print(f"DropPath: max={drop_path_max:.3f}, per-layer rates={[round(r, 4) for r in dpr]}")
