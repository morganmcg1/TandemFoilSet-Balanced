...
class DropPath(nn.Module):
    ...
    During training, with probability ``drop_prob`` the entire residual branch
    output is replaced by zero (per-sample). Kept paths are rescaled by
    ``1/(1-drop_prob)`` so expectation is preserved. At eval this is the identity.
    """
...
class Transolver(nn.Module):
    def __init__(self, space_dim=1, n_layers=5, n_hidden=256, dropout=0.0,
        ...
        dpr_schedule = [drop_path_rate * i / max(n_layers - 1, 1) for i in range(n_layers)]
