...
class SwiGLUMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.w_down((gate * gate) * self.w_up(x))   # H45: Squared ReLU gate (Primer, So et al. 2021)
...
# H45: Squared ReLU gate sanity check (Primer, So et al. 2021)
_h45_test_x = torch.tensor([-1.0, 0.0, 0.5, 1.0, 2.0])
_h45_gate_vals = F.relu(_h45_test_x).pow(2)
print(f"[H45] Squared ReLU at [-1, 0, 0.5, 1, 2]: {_h45_gate_vals.tolist()}")
print(f"[H45] Expected:                            [0.0, 0.0, 0.25, 1.0, 4.0]")
print(f"[H45] SwiGLU inner_dim: {model.blocks[0].mlp.inner_dim}, n_params: {n_params}")
