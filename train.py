...
class DropPath(nn.Module):
    ...
    During training, drops the entire branch output with probability `drop_prob`
    (replacing it with zero) and scales the surviving paths by 1/(1 - drop_prob).
    At inference, identity.
    """
...
class Transolver(nn.Module):
    def __init__(self, space_dim=1, n_layers=5, n_hidden=256, dropout=0.0,
        ...
        # Linear stochastic depth schedule: 0 at layer 0, drop_path_rate at last layer
...
def evaluate_split(model, loader, stats, surf_weight, device, fourier_enc=None) -> dict[str, float]:
    ...
    with torch.no_grad():
        for x, y, is_surface, mask in loader:
            ...
            # Defensively drop samples with non-finite ground truth from this batch
            # so the inf in y doesn't propagate into loss/MAE via inf*0=nan when the
            # accumulator zeroes the masked positions. (test_geom_camber_cruise has
            # one such sample; the scoring helper's y_finite check is bypassed by
            # IEEE arithmetic.)
            B = y.shape[0]
            y_finite_sample = torch.isfinite(y.reshape(B, -1)).all(dim=-1)  # [B]
            if not y_finite_sample.all():
                mask = mask & y_finite_sample.view(B, 1)
...
class Config:
    ...
    drop_path_rate: float = 0.0  # max DropPath rate in linear schedule (0 at layer 0)
