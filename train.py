...
# Fourier encoding replaces 2 scalar camber dims (15, 19) with (1 + 2*n_freqs)
# dims each, growing fun_dim by 2 * 2 * n_freqs. Default n_freqs=4 → 22 + 16 = 38.
...
if best_metrics:
    ...
    if not cfg.skip_test:
        ...
        # Nansafe aggregate: ignore splits where the metric is non-finite
        # (data/scoring.py has a cruise idx-20 -inf bug that poisons in-tree
        # test_avg; cohort policy is to also log a nansafe variant).
        import math as _m
        test_avg_nansafe: dict[str, float] = {}
        keys = set().union(*[m.keys() for m in test_metrics.values()])
        for k in keys:
            vals = [m[k] for m in test_metrics.values() if k in m and _m.isfinite(m[k])]
            if vals:
                test_avg_nansafe[f"avg/{k}"] = sum(vals) / len(vals)
        print(f"\n  TEST  avg_surf_p={test_avg.get('avg/mae_surf_p', float('nan')):.4f}  "
              f"nansafe_avg_surf_p={test_avg_nansafe.get('avg/mae_surf_p', float('nan')):.4f}")
        ...
        for k, v in test_avg_nansafe.items():
            test_log[f"test_avg_nansafe/{k.split('/', 1)[1]}"] = v
        wandb.log(test_log)
