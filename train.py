...
optimizer = Lion(
    ...
    lr=cfg.lr,
    weight_decay=cfg.weight_decay,
    betas=(0.9, 0.999),  # β₂-up: 0.99 → 0.999 (long-horizon memory probe, PR #2647)
)
print(f"Optimizer: Lion (Chen et al. 2023) | lr={cfg.lr}, wd={cfg.weight_decay}, betas=(0.9, 0.999) | sign-based momentum update | replaces AdamW")
print(f"[DIAG] Lion betas: {optimizer.defaults['betas']}")
print(f"[DIAG] Lion lr: {optimizer.defaults['lr']}, wd: {optimizer.defaults.get('weight_decay', optimizer.defaults.get('wd'))}")
print(f"[DIAG] β₂ memory window: ~{1/(1-optimizer.defaults['betas'][1]):.0f} steps")
print(f"[DIAG] Total model params: {sum(p.numel() for p in model.parameters()):,}")
warmup_epochs = 3
