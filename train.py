...
def evaluate_split(model, loader, stats, surf_weight, device) -> dict[str, float]:
    ...
    with torch.no_grad():
        for x, y, is_surface, mask in loader:
            ...
            # smooth_l1 (Huber, beta=0.20) — train/eval shape parity with the
            # training step so val/loss is comparable.
            per_elem = F.smooth_l1_loss(pred, y_norm, beta=0.20, reduction="none")
...
for epoch in range(MAX_EPOCHS):
    ...
    for step_idx, (x, y, is_surface, mask) in enumerate(
        ...
        with torch.amp.autocast("cuda", dtype=torch.bfloat16, enabled=cfg.amp):
            ...
            # smooth_l1 (Huber, beta=0.20) in float32 for numerical stability.
            ...
            per_elem = F.smooth_l1_loss(
                pred.float(), y_norm.float(), beta=0.20, reduction="none"
