...
class Config:
    ...
    epochs: int = 50
    feature_noise_std: float = 0.01     # train-time Gaussian noise std (semantics-aware)
...
for epoch in range(MAX_EPOCHS):
    ...
    for x, y, is_surface, mask in tqdm(train_loader, desc=f"Epoch {epoch+1}/{MAX_EPOCHS}", leave=False):
        ...
        if cfg.feature_noise_std > 0.0:
            ...
            # Per-sample noise on dims 13..23 (log_re, AoA*, NACA*, gap, stagger): one
            # draw per sample, broadcast across N nodes so the (geometry, flow) condition
            # stays consistent within each forward pass.
