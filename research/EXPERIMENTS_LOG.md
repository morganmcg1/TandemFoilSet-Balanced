...
## 2026-05-13 19:35 — PR #2355 (CLOSE): frieren batch_size=1 — FAIL +20.59%, batch_size axis FULLY BRACKETED

- Branch: `willowpai2g48h5-frieren/batch-size-1`
- Hypothesis: continue opt-step density push from bs=2 → bs=1 (4× from baseline, ~1.5× from #2247)
- W&B run: `tv2jhdxv`

| Metric | bs=1 | #2247 baseline (bs=2) | Δ |
|---|---:|---:|---:|
| val_avg/mae_surf_p | **56.2913** | 46.6788 | **+9.61 (+20.59%)** |
| test_avg/mae_surf_p | **47.7256** | 39.7696 | **+7.96 (+20.00%)** |

Per-split test (all 4 splits regress uniformly, camber_cruise worst at +29.93%):
- in_dist: 53.76 (+22.07%), camber_rc: 59.72 (+12.43%), camber_cruise: 31.37 (+29.93%), re_rand: 46.05 (+21.91%)

**Diagnostics:**
| Metric | bs=1 | bs=2 #2247 | Direction |
|---|---:|---:|---|
| clip_rate | 99.46% | 94.70% | **OPPOSITE** of prediction |
| grad_norm_mean (final) | 19.7 | n/a | descending from 72.3 |
| grad_norm_max | 363.3 | 85.4 (p99) | 4× higher |
| EMA-live gap (val) | −13.19 | −4.71 | wider |
| Best epoch | 21/50 | 34/50 | fewer epochs (contention) |

**Mechanism:** pr predicted "per-sample gradients → more un-clipped steps." Opposite happened. At bs=1, per-sample gradients are NOT pre-averaged → L2 norm is systematically larger than bs=2 batch average → clip rate climbed 94.70%→99.46%. Pattern v3 confirmed for 7th axis: any axis that increases time-in-high-gradient-regime adds clipping.

**Throughput contention:** First 12 epochs at ~107s/ep (GPU contention from another fleet process). Recovered at epoch 13 to ~54s/ep. Even disentangling: clean-throughput-only trajectory (ep 13-21: val 74.0→56.3) still far behind bs=2 trajectory at matched opt-step count. Failure is mechanism-driven, not throughput-driven.

**Decision: CLOSE.** Batch_size axis is now FULLY BRACKETED: [bs=1 FAIL +20.59% / bs=2 OPT / bs=4 FAIL]. Pattern v3 confirmed for 7th time.

→ Closed 2026-05-13 19:35 UTC.

## 2026-05-13 19:35 — PR #2219 (MERGE): alphonse n_hidden=160 retest on 15-compound stack — 16th compound winner

- Branch: `willowpai2g48h5-alphonse/n-hidden-160-width-floor`
- Hypothesis: n_hidden=160 wins vs n_hidden=192 via throughput-vs-capacity tradeoff: narrower model → faster epochs → more cosine schedule decay in 30-min budget → better convergence. Original standalone win confirmed at old stack (#2219 v1: val 50.5152, W&B `smqwihpd`). Compound retest at 15-compound stack (bs=2 + huber_beta=0.25 baked in).
- W&B run: `741bdhcl` (canonical 15-stack); informational 14-stack: `560twhbv`

| Metric | n=160 (15-stack, bs=2) | #2247 baseline | Δ |
|---|---:|---:|---:|
| val_avg/mae_surf_p | **45.9186** | 46.6788 | −0.76 (**−1.63%**) |
| test_avg/mae_surf_p | **39.0381** | 39.7696 | −0.73 (**−1.84%**) |

Per-split test (EMA, epoch 38):
| Split | n=160+bs=2 | #2247 | Δ% |
|---|---:|---:|---:|
| in_dist | 42.2300 | 44.0421 | **−4.12%** |
| camber_rc | 53.9414 | 53.1169 | +1.57% (slight regression) |
| camber_cruise | 23.4382 | 24.1470 | **−2.93%** |
| re_rand | 36.5427 | 37.7723 | **−3.26%** |
| **test_avg** | **39.0381** | **39.7696** | **−1.84%** |

**Diagnostics:**
| Metric | n=160+bs=2 | n=160+bs=4 (v1) |
|---|---:|---:|
| Param count | 650,767 (0.65M) | 650,767 |
| Epoch time | 47.4s | 48.7s |
| Epochs reached | 38/50 | 38/50 |
| Cosine LR at termination | 7.04e-5 (14.1% base) | 7.05e-5 (same) |
| Grad clip rate | 98.06% | 97.99% |
| Grad norm mean | 23.63 | 18.04 (+31% — bs=2 noise) |
| Opt-steps | 28,500 | 14,250 (2× from bs=2) |
| EMA-live gap (test) | −1.55 | −1.91 |

**Key findings:**
1. n_hidden=160 win is specifically a **bs=2 × n_hidden=160 interaction**. The informational 14-stack run (n=160, bs=4, β=0.25) was val 51.5954 — **slight regression** vs 14-stack baseline (50.38). The axis win doesn't compound with β=0.25 alone; it requires bs=2.
2. **Narrower net counter-balances bs=2 gradient noise.** At bs=2, grad norm mean jumps from 18.04 → 23.63 (+31%). The narrower model (0.65M vs 0.93M) has tighter per-param direction-variance which partially absorbs the per-step noise amplification.
3. **camber_rc moved slightly backward** (+1.57%). Width is not the right lever for this hardest OOD split. Pattern: camber_rc responds most to regularization axes (wd=3e-4 cracked it −1.38%), not throughput axes.

**Compounding decomposition vs old #1982 baseline:**
- n=160 alone: −4.04% val / −2.99% test
- Huber β=0.25 alone: −4.29% val
- bs=2 alone: −7.35% val / −9.04% test
- Composite (this run vs #1982): −12.78% val / −13.21% test (sub-additive, as mechanisms partially share "late-phase refinement" lever)

**Decision: MERGE** — 16th compound winner. Width axis now: [?, 160 WIN, 192 OPT@old-stack, 224 FAIL, 256 FAIL].

→ Merged 2026-05-13 19:35 UTC. New baseline: val=45.9186 / test=39.0381.

## 2026-05-13 19:35 — PR #2305 (SEND BACK): tanjiro wd=3e-4 compound retest — wins at 15-stack, needs n_hidden=160 compound retest

- Branch: `willowpai2g48h5-tanjiro/weight-decay-3e-4`
- W&B run: `g9qqasuo` (15-stack, bs=2); informational 14-stack: `agci2137`

| Metric | wd=3e-4 (15-stack, bs=2) | #2247 baseline | Δ vs baseline |
|---|---:|---:|---:|
| val_avg/mae_surf_p | 46.0794 | 46.6788 | −0.60 (−1.28%) ✓ |
| test_avg/mae_surf_p | 39.2013 | 39.7696 | −0.57 (−1.43%) ✓ |

vs NEW #2219 baseline (45.9186/39.0381): val +0.33% (+0.16 abs), test +0.43% (+0.16 abs) — just misses.

**Result:** wd=3e-4 wins at 15-stack (n=192+bs=2) but doesn't beat 16-stack (n=160+bs=2) baseline from #2219 which merged first.

**Key observation:** OOD pattern REDISTRIBUTED at bs=2. At 14-stack, wd=3e-4 was OOD-dominant (re_rand −2.72%, camber_rc −1.38%, in_dist neutral). At 15-stack: in_dist is strongest gain (−3.54%), camber_rc goes flat (+0.05%). bs=2 already pulled OOD splits near floor; wd=3e-4 now picks up remaining gain on in_dist.

**Additive prediction confirmed precisely:** predicted val 46.12/test 39.21, observed val 46.08/test 39.20. Mechanisms compound at arithmetic level.

**Clip rate clarification:** wd=3e-4 RAISED clip rate from 94.70% (bs=2 baseline) → 99.17%. Tighter wd → smaller param norms → model "fights harder" per step → higher norms → more clipping. Yet val/test still improved — confirming wd axis bypasses clip-saturation gating in BOTH directions.

**Decision: SEND BACK** for n_hidden=160 + wd=3e-4 compound retest. Additive prediction: val ≈ 45.33, test ≈ 38.48.

→ Sent back 2026-05-13 19:35 UTC. New target: val < 45.9186, test < 39.0381. Reproduce: `--n_hidden 160 --n_layers 3 --batch_size 2 --epochs 50` with wd=3e-4 verified.

## 2026-05-13 19:35 — New assignments: alphonse #2395 (n_hidden=128), frieren #2398 (mlp_ratio=4 at n=160)

**alphonse PR #2395: n_hidden=128 width-floor push**
- Hypothesis: continue width-narrowing from n=160 (16-compound winner). n=128 → even faster epochs → even more cosine decay in 30-min budget. Risk: capacity floor bites on hardest OOD splits (camber_rc already showing +1.57% at n=160).
- Param count: ~0.42M (vs 0.65M at n=160, −36%)
- Predicted epoch time: ~35-42s (further reduction from n=160's 47.4s)
- Reproduce: `--n_hidden 128 --n_layers 3 --batch_size 2 --epochs 50`
- Target: val < 45.9186, test < 39.0381

**frieren PR #2398: mlp_ratio 2 → 4 at n=160**
- Hypothesis: alphonse's n=160 win freed 0.28M params (0.93M → 0.65M). Reinvest those into deeper FFN (mlp_ratio 2→4: FFN hidden 320 → 640). Attention is token-count-bound so per-epoch time increase should be modest (~52-55s vs 47.4s). Param count grows back to ~0.96M but with n=160 throughput advantage preserved.
- Key question: is the FFN (not attention width) the binding capacity constraint at n=160?
- Reproduce: `--n_hidden 160 --n_layers 3 --batch_size 2 --mlp_ratio 4 --epochs 50`
- Target: val < 45.9186, test < 39.0381
