...
- **🚨 CRITICAL CORRECTION (from #2689 frieren empirical M-value count)**: The train set CONTAINS 75 samples at M=0.778 and 71 at M=0.889 (146 total in the supposed "rc OOD range")! **The rc OOD axis is NOT pure ch15-M extrapolation** as I had asserted across 6+ rounds. fern's earlier "Train tandem ch15 = {0, 0.111, ..., 0.667, 1.0}" claim was either about a subset or had a different channel reference. **The rc-defining condition is multi-dimensional** — likely a joint (M_front, M_back, P_front, P_back, Re, AoA) interaction. Most strategic decisions in rounds 22-01:05 were grounded on the single-axis-M-extrapolation theory and need re-grounding. #2703 fern asymmetric ch15 jitter is still valuable (general augmentation toward higher M values is benign), but the framing was wrong. The "structural cruise/rc dichotomy" observation (7 experiments) is still REAL — the explanation needs revision. Round 01:54 includes a DIAGNOSTIC experiment (#2721 frieren rc-NN-oversampling + split-signature dump) to characterize the actual OOD axis.

- Round 01:54 closures + analysis:
  - **#2689 frieren shape-bin-oversampling-m05**: +6.0% val. Single-axis M-density did NOT help rc (+1.2%) even at 71% high-M mass; effective undersampling of M<0.5 hurt iid bulk (+10.6% in-dist). Confirms rc OOD is multi-axis.
  - **#2688 nezuko surface-only-aux-head**: +3.66% val. Aux objective converged faster/cleaner (loss 0.039 vs #2660's 0.062; pred_mag 0.988 vs 0.980) BUT main metrics regressed in same shape as #2660 → volume-pollution NOT the cause; the real cost is **representation-capacity competition at the post-LN trunk hidden state**. Aux-output-head meta-axis FULLY CLOSED across 2 arms (#2660 vol+surf, #2688 surf-only).
  - **#2599 tanjiro / #2534 edward**: stale 24+ hours, pods on rate-limit cycle, closed and reassigned.

- Round 01:54 strategic plan:
  - **#2721 frieren rc-NN-oversampling-multi-axis** (DIAGNOSTIC + corrective): build per-split feature signature table (committed JSON), compute per-train k-NN distance to rc samples in full (M, P, T, Re, AoA) space, oversample most-rc-similar 20% by up to 3× without undersampling iid bulk. Unlocks future strategy by revealing actual rc-defining axis.
  - **#2723 nezuko ensemble-output-heads-k3**: deep-ensemble-style averaging from K=3 parallel output heads sharing trunk; targets variance reduction on rc/re_rand specifically; ~1.5K added params, no extra trunk compute.
  - **#2724 tanjiro geometry-mirror-TTA**: exact y-symmetry test-time averaging (flip y-coord + AoA, predict, un-flip Uy, average). Edward's #2534 TTA validated mechanism (-0.04 val / -0.12 test on Re bracket); mirror should give larger gain. Free training, ~2× val compute.
  - **#2725 edward multi-seed-variance-new-baseline**: 2 seeds (42, 43) of new baseline stack with TTA-val-per-epoch checkpoint selection. Establishes the run-to-run noise floor on the new baseline (required for all future merge decisions); validates whether TTA wrapper stacks above baseline reliably.
...
| PR | Student | Slug | Status | Priority | Notes |
|----|---------|------|--------|----------|-------|
| #2699 | alphonse | `re-conditional-layerscale` | WIP | **HIGHEST** | 5th Re-conditioning hook: per-block LayerScale α(Re)=1.0+g(logRe) gating residual additions. ~250 params, zero-init residual. |
| #2702 | askeladd | `swa-last-5-epochs` | WIP | **HIGH** | Optimizer-level flat-minima: PyTorch built-in SWA on epochs 24-28. Orthogonal to all falsified architecture/feature/loss directions. |
| #2703 | fern | `asymmetric-ch15-jitter-sigma05` | WIP | **HIGH** (re-graded) | One-sided positive ch15 jitter σ=0.05. Still potentially valuable (pushes samples toward higher M values) but framing was on the questioned single-axis-M-OOD theory. |
| #2690 | thorfinn | `re-conditional-output-bias` | WIP | **HIGH** | 4th Re-conditioning hook — additive per-channel bias after ReScaleHead. Same family as #2699 but at output-bias injection point. |
| #2721 | frieren | `rc-nn-oversampling-multi-axis` | WIP (just-assigned) | **HIGHEST** | **DIAGNOSTIC + corrective**: builds per-split feature signature table (commits split_signatures.json) + uses k-NN-to-rc weighting on full (M, P, T, Re, AoA) signature instead of single ch15. Re-grounds the OOD axis. |
| #2723 | nezuko | `ensemble-output-heads-k3` | WIP (just-assigned) | **HIGH** | K=3 parallel output heads with shared trunk, mean prediction. Deep-ensemble-style variance reduction targeting rc/re_rand specifically. |
| #2724 | tanjiro | `geometry-mirror-tta` | WIP (just-assigned) | **HIGH** | Exact y-symmetry TTA: flip y-coord + AoA, predict, un-flip Uy, average. Stacks with edward's Re-bracket TTA mechanism. Free training, ~2× val compute. |
| #2725 | edward | `multi-seed-variance-new-baseline` | WIP (just-assigned) | **HIGH** | 2 seeds (42, 43) of new baseline stack with TTA-val-per-epoch checkpoint selection. Establishes variance floor for all future merge decisions. |
...
| #2690 | thorfinn | re-conditional-output-bias | 27.5868 | **−2.32%** | **−76.5%** |
...
- **re-conditional-attn-temperature** (#2699, #2768 current): LayerScale (#2699) closed (+5.88% vs new baseline) — mechanism real but 2 epochs short (memory overhead) and block0 gate magnitude |α−1|=0.475 disrupts early features. **Pattern: per-block gating is too heavy for 30-min budget.** Reassigned to Re-cond attention temperature (#2768, 10 params) which avoids residual-stream gating overhead.
- **swa-last-5-epochs** (#2702): SWA works (-2.62% vs regular best) but swa_lr=1e-4 switches LR UP from cosine floor — undermining critical cosine-tail refinement. SWA meta-axis CLOSED.
- **ensemble-output-heads-k3** (#2723): Heads anti-correlated (pairwise cosine -0.14 to -0.17 = competition not independence). +5.61% vs new baseline. Ensemble-output-head meta-axis CLOSED.
- **asymmetric-ch15-jitter-sigma05** (#2703): One-sided positive jitter pushed cruise samples OUTSIDE their tight cluster (+15.1% cruise). ch15-augmentation-family CLOSED.
- **naca-jitter-ch16-17-only-sigma02 (mask ch15)** (#2662): +7.04% val vs new baseline #2650. **Establishes ch15 was the ENTIRE active axis in #2625** — symmetric jitter on ch16+ch17 alone is near-noop (val_avg drift +1.6% on cruise, +5.6% on rc, +4.2% on re_rand). Both the -43.7% cruise win AND +24.2% rc regression of #2625 evaporated together when ch15 was masked. **ch15 simultaneously serves as cruise interpolation axis AND rc OOD extrapolation axis** with opposite effects — confirming the structural dichotomy mechanism. Symmetric ch15 jitter densifies inward (helps cruise interpolation, hurts rc extrapolation). Reassigned to #2703 fern asymmetric-ch15-jitter-sigma05 (one-sided positive: PUSH outward toward rc OOD values rather than densify inward). **⚠️ Note**: The "ch15 OOD beyond train cluster" framing was challenged by frieren #2689 — train DOES contain M=0.778/0.889 samples (146 total). The cruise/rc dichotomy is real but the explanation needs updating.
- **shape-bin-oversampling-m05** (#2689): +6.0% val vs new baseline. Single-axis M≥0.5 oversampling at 3× did NOT help rc (+1.2%) even at 71% high-M mass; effective undersampling of M<0.5 hurt iid bulk (val_single_in_dist +10.6%, cruise +6.6%, re_rand +8.5%). **🚨 critical empirical correction**: train set contains 75 samples at M=0.778 and 71 at M=0.889 — contradicting the single-axis-M extrapolation theory. rc OOD axis is multi-dimensional (likely joint M_front, M_back, P, Re, AoA condition). Single-axis data-side reweighting is not the right lever. Reassigned to #2721 rc-NN-oversampling-multi-axis (diagnostic + multi-axis k-NN reweighting).
- **surface-only-aux-normal-head** (#2688): +3.66% val vs new baseline. Aux objective converged FASTER and CLEANER than #2660 (aux_loss 0.039 vs 0.062; pred_mag 0.988 vs 0.980) while main metrics regressed in IDENTICAL shape (cruise +1.5%, rc +2.4%, re_rand +6.5% — virtually same as #2660 vs new baseline). **Mechanism: representation-capacity competition at the post-LN trunk hidden state, NOT volume-target pollution.** Adding aux supervision at the final injection point steals capacity from bulk-flow encoding; cost concentrates on regimes where bulk-flow features dominate (re_rand, rc). **Aux-output-head meta-axis FULLY CLOSED across 2 arms** (#2660 vol+surf +3.13%, #2688 surf-only +3.66%). Reassigned nezuko to #2723 ensemble-output-heads-k3 (deep-ensemble variance reduction, fundamentally different output-side mechanism).
- **se-channel-attention-r8** (#2599 tanjiro stale): Closed as STALE after 24+ hours without comments or results. Pod stuck on rate-limit cycle. SE-channel-attention hypothesis remains untested by this branch. Reassigned to #2724 geometry-mirror-TTA.
- **tta-re-bracket (multi-seed follow-up)** (#2534 edward stale): Closed as STALE after 5+ hours without new arm following advisor request for multi-seed + per-epoch TTA-val. Original arm (val_avg 29.86 on a +1.0 variance no_tta run) was vs OLD baseline #2011 — would be ~+5.7% vs new baseline even with TTA. TTA mechanism itself cleanly validated (consistent -0.04 val / -0.12 test, σ=0.05 sweet spot). Reassigned edward to #2725 multi-seed-variance-new-baseline (proper variance floor characterization on the new baseline stack).
