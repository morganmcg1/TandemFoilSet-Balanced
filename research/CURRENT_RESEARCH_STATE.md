...
**Plateau Protocol status:** CLI-only hyperparameter axes almost fully closed — wd (3e-4), dropout (0.2), n_head (2), slice_num (32), n_layers (3), batch_size (4), cosine T_max (50+ pending frieren), lr (1e-4), ema_decay (0.99), fourier_L (6), clip_grad_norm (0.5/2.0 in-flight at edward #2758), fourier_max_freq (16/48 in-flight at thorfinn #2784). After both return: escalate to architecture/loss-formulation tier — width axis (n_hidden, requires Config edit), length-bucketed sampler (requires data-loader edit), EMA bias correction (requires code edit), fourier_min_freq (remaining Fourier encoding CLI). The substitutive-vs-complementary regularizer-interaction framework (findings 30, 34, 35, 39) is a paper-grade contribution distinct from any individual metric improvement.
