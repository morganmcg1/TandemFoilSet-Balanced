...
|----|---------|--------|----------|--------|
| **TBD** | **edward** | **Lion β2 sweep (0.95/0.999, code-edit tier)** | NEW | Idle (assigning) |
| **#2784** | **thorfinn** | **fourier_max_freq=16/48 sweep at L=6 on new compound** | NEW | WIP |
| **TBD** | **frieren** | **drop_path stochastic depth sweep (code-edit tier)** | NEW | Idle (assigning) |
...
48. **FOURIER_MIN_FREQ=1.0 LOCKED, BOTH DIRECTIONS CLOSED (#2789):** min_freq=0.5 (val=43.01, +2.4%) and min_freq=2.0 (val=42.50, +1.2%) both regress. With L=6 log-spaced [1.0, 32.0], f₀=1.0 ≈ one full half-domain-period — wider (0.5) → near-linear feature duplicating learned bias, narrower (2.0) → loses globally-coherent positional feature. **Critical: both arms regress re_rand 8-9%** (band-coverage drives Re-generalization). Both arms IMPROVE single_in_dist 4-5% — model has spare in-dist capacity while under-fitting OOD splits. Fourier band now fully closed on shape (L=6) and position (f₀=1.0). max_freq companion (#2784) still in-flight. Closes the fourier band at the (1.0, 32.0) range.

47. **N_HIDDEN=128 LOCKED, BOTH DIRECTIONS CLOSED — FIRST CODE-EDIT-TIER EXPERIMENT (#2793):** n_hidden=96 (val=45.83, +9.1%, test +6.3%) and n_hidden=160 (val=43.32, +3.1%, test +1.5%) both regress on fixed 30-min budget. **Asymmetric mechanism:** narrower (96, per-head-dim 48) loses per-head capacity floor below finding-38 sweet spot of 64 (16% faster s/ep, 41 epochs, but never catches up — capacity dominates). Wider (160, per-head-dim 80) loses budget — only 29 epochs at 18% slower s/ep but val still descending at cap (slope ~0.5/ep) — budget-limited not architecture-limited. **Critical contrast with finding 37 speed-dividend:** for width, per-head-dim quality floor dominates; for depth/slices, padding-dominated per-step compute lets speed dividend win. Two-line Config edit (`n_hidden: int = 128` + `n_hidden=cfg.n_hidden`) — code-edit pattern established for future Config additions.
...
**Plateau Protocol status: ESCALATED TO CODE-EDIT TIER (round 2).** All CLI axes confirmed closed — wd (3e-4), dropout (0.2), n_head (2), slice_num (32), n_layers (3), batch_size (4), cosine T_max (50), lr (1e-4), ema_decay (0.99), fourier_L (6), clip_grad_norm (1.0), fourier_min_freq (1.0). **9 consecutive CLI/code-edit negatives = deep plateau.** Code-edit tier round 2:
- **In-flight:** thorfinn #2784 (fourier_max_freq=16/48 — CLI-tier final companion)
- **Assigning:** edward → Lion β2 sweep (β2=0.95/0.999, Config edit on optimizer construction); frieren → drop_path stochastic depth (Block code edit, targets re_rand OOD regularization)
- **Queued:** linear-warmup+cosine-decay-to-zero scheduler, length-bucketed sampler (data-loader edit), EMA bias correction, SwiGLU MLP swap, auxiliary Re-prediction head
- **Cross-cutting finding:** re_rand consistently identified as deciding split (findings 41, 43, 48). OOD regularization is the highest-EV next-axis target.
...
- **#2789 (frieren):** fourier_min_freq=0.5/2.0 sweep on n_layers=3+wd=3e-4 — CLOSED. min_freq=1.0 confirmed local optimum. Wider/narrower both regress; both arms hurt re_rand 8-9% (band-coverage critical for Re-generalization). Both arms IMPROVE single_in_dist (4-5%) — model has spare in-dist capacity. Finding 48. Frieren reassigning to drop_path stochastic depth (code-edit tier).
- **#2793 (edward):** n_hidden=96/160 width sweep — CLOSED. n_hidden=128 locked as fixed-budget optimum. Below: per-head-dim 48 < finding-38 sweet spot 64 (speed dividend can't compensate). Above: per-head-dim 80, val still descending at cap (29 epochs) but budget-limited. Speed-dividend (finding 37) does NOT apply to width axis. First successful code-edit-tier experiment. Finding 47. Edward reassigning to Lion β2 sweep (code-edit tier).
