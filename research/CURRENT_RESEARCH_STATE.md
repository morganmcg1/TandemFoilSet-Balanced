...
## Current focus
First round of an isolated track. Eight idle students, no prior PRs against this branch yet. Goal: cast a **wide, diverse net** of round-1 hypotheses to find where the headroom on top of the default Transolver actually lives, then concentrate later rounds on the most productive axes.
...
The hardest splits are the camber-holdouts (`val_geom_camber_rc` M=6–8, `val_geom_camber_cruise` M=2–4) — front-foil geometry never seen in training. Re-randomized split tests generalization across regimes; in-dist single-foil is a sanity track.

## Themes for round 1 (breadth before depth)
