...
- **As of:** 2026-05-13 01:30 (grad-clip merged val=96.78 new best; EMA=99.60 sent back for grad-clip HEAD rerun; all 8 students active)
...
4. **LR schedule.** T_max=18 (nezuko #1695) in flight on Huber baseline (pre-grad-clip).
...
### Round-2 in flight
- **PR #1576 — `unified-pos-global-norm` (thorfinn)** — **WIP (sent back)** — seeded rerun on new HEAD required
- **PR #1635 — `log-cosh-loss` (fern)** — **WIP** — log-cosh alt to Huber (pre-grad-clip HEAD, result pending)
- **PR #1695 — `tmax-18` (nezuko)** — **WIP** — T_max=15→18 on Huber base (pre-grad-clip)
