...
|---|---|---|---|---|
| **#2297** | **askeladd** | wip (new) | Lion lr sweep {2e-4, 4e-4, 5e-4} | Fine-bracket the winning lr=3e-4 |
...
5. **SWA frac bounded below** — only frac≥0.75 averages in flat-loss region. EMA (#2285) is the clean fix.
...
4. **Lion + longer T_max** — cosine schedule T_max=15 may not be optimal for Lion's different convergence profile. T_max=12 (faster cooling) or T_max=20 (slower, more epochs matter).
5. **σ=0.5 on Lion stack** — if thorfinn's σ=0.5 shows promise on β=0.3, apply to Lion baseline too
6. **EMA + Lion** — EMA (#2285) on β=0.3 baseline; after confirming EMA works, test on Lion stack
...
- GC (#2240 frieren), β=0.2 (#2243 edward), ReZero γ=1.0 (#2269 fern), max_norm {0.75,1.0} (#2270 alphonse), EMA (#2285 tanjiro) — all need Lion rebase when done
