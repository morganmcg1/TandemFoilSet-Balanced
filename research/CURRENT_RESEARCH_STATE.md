...
## Active Experiments (round 23:41 — channel-selective shape augmentation + multi-task aux output)

| PR | Student | Slug | Status | Priority | Notes |
|----|---------|------|--------|----------|-------|
| #2650 | alphonse | `re-conditional-layernorm-affine` | WIP | **HIGH** | Re-conditional LN γ/β (shared, zero-init) — canonical Conditional-InstanceNorm pattern at the LN-affine injection point; bounded modulation by construction (LN normalizes before γ/β applies) |
...
| #2599 | tanjiro | `se-channel-attention-r8` | WIP (pod rate-limit stuck) | MEDIUM | SE channel-attention after each block, zero-init residual gate — global-context modulation pathway. **Tanjiro pod still on secondary rate-limit cycle iter 256+; PR labels correct but pod can't poll. Will recover naturally.** |
| #2659 | frieren | `sample-level-focal-mae-gamma1` | WIP | **HIGH** | Per-sample focal weighting γ=1 (linear, NOT γ=2 quadratic that saturated clamp in #2622). Averages elements before applying focal weight → avoids heavy-tailed element saturation. |
| #2660 | nezuko | `surface-normal-auxiliary-output-head` | WIP | **HIGH** | Auxiliary output head predicting (n_x, n_y) per surface element — Kendall/Gal/Cipolla 2017 multi-task. Forces trunk to develop normal-aware representations rather than reading normals from input channel (after #2624 input curvature closed). |
| #2662 | fern | `naca-jitter-ch15-17-only-sigma02` | WIP | **HIGHEST** | **Channel-selective** NACA jitter: σ=0.02 on ch 15 (camber amp) + ch 17 (thickness), **masking ch 16 (camber position = rc OOD axis)**. Tests the Round 23:41 KEY FINDING — preserves the cruise -43.7% win from #2625 without the rc +24% regression. |
| #2626 | thorfinn | `per-channel-separate-heads-kaiming-h128` | WIP (re-run) | **HIGH** | Re-issued with kaiming_normal init (replacing zero-init) and head_hidden=128 (from 64). Original run was under-converged — heads needed to be initialized to non-zero outputs. Cruise -44% / rc +21% dichotomy from initial run matched #2625 — channel-selective approach should help here too. |
| #2534 | edward | `tta-re-bracket` | WIP (active, in progress) | MEDIUM | Per-epoch TTA-val for checkpoint selection + multi-seed run to bound variance |
...
- **naca-feature-jitter-sigma-0p02 (3-channel)** (#2625): +3.45% val / +? test BUT **DRAMATIC DICHOTOMY**: val_geom_camber_cruise **-43.71% MASSIVE WIN** ✅ / val_geom_camber_rc **+24.20% catastrophic** ❌ / val_single_in_dist +11.59% / val_re_rand -0.96%. **Cross-confirmation from #2626 thorfinn per-channel-heads (cruise -44% / rc +21%)** — two independent experiments showed parallel cruise-WIN/rc-LOSS splits. **🔑 KEY FINDING: rc camber-position is an EXTRAPOLATION problem (test values lie outside train distribution on ch 16); cruise is INTERPOLATION (within train).** Implication: **input-side shape augmentation must be channel-selective** — augment only channels NOT held out in the test splits, never augment along the OOD axis itself. Full-channel jitter CLOSED; channel-selective approach (#2662 fern, mask ch 16) tests the structural insight.
