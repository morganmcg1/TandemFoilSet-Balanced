...
|----|---------|-------|--------|-----------------|
| #3884 | alphonse | Batch size scaling on bf16: batch=6 and 8 vs baseline 4 | WIP — NEW | bf16 freed 9 GB; bigger batch tests signal-quality vs step-count tradeoff |
| #3887 | edward | Cosine T_max refinement: bracket T_max=30 with T_max=25 and 40 | WIP — NEW | #3822 winner maps to the knee; "above 30" is student's own prediction |
| #3725 | fern | Per-group grad-clip: attention (max_norm=1.0) vs MLP (5.0/10.0) | WIP — STALE | GitHub rate-limit blackout delayed; target now 56.00 (T_max=30 required) |
| #3726 | frieren | Lion weight decay sweep: wd=1e-3 and 3e-3 (Lion paper rec.) | WIP — STALE | Same rate-limit delay; target now 56.00 |
| #3674 | nezuko | Per-channel pressure weight: w_p=0.5 and 2.0 vs 1.0 | WIP — STALE | Oldest open WIP; target now 56.00 |
| #3731 | tanjiro | Signed log1p on pressure: direct asinh competitor (v2) | WIP — STALE | Rate-limit delay; target now 56.00 |
| #3776 | askeladd | EMA decay ablation on bf16 stack: 0.997/0.995 (v2) | WIP — STALE | Rate-limit delay; target now 56.00 |
| #3734 | thorfinn | SwiGLU gated activation in TransolverBlock MLPs (v2) | WIP — STALE | Rate-limit delay; target now 56.00 |

**Note:** PRs #3725, #3726, #3674, #3731, #3776, #3734 were affected by a GitHub API rate-limit blackout (09:05-09:20Z). All pods confirmed training at 85-94 GB GPU utilization before the blackout. Results may be sitting on local pod filesystems awaiting commit.
...
3. **Is MLP gradient over-clipped?** (#3725 fern) — 100% clip rate at norms 25-180; per-group clip
4. **Is Lion weight decay too low?** (#3726 frieren) — wd=1e-3 and 3e-3 vs current 3e-4
5. **Does per-channel pressure weight matter?** (#3674 nezuko) — w_p=0.5 vs w_p=2.0
6. **Does signed log1p beat asinh?** (#3731 tanjiro) — more aggressive tail compression
7. **Does EMA decay=0.997 or 0.995 still beat 0.999 on 7-mech stack?** (#3776 askeladd) — convergence-horizon hypothesis revisited with T_max=30
...
- **WeightedRandomSampler**: inverse-error reweighting after epoch 1 — orthogonal to all 7 mechanisms
- **Channel-decoupled output heads**: separate MLP for Ux/Uy vs p (pressure has very different statistics)
- **LR × T_max coupling**: if T_max=30 is the answer, what's the optimal lr_init under this schedule?
- **EMA decay with T_max=30**: askeladd #3776 will answer — T_max=30 changes convergence horizon
