...
- **Date:** 2026-05-16 (~08:05 UTC) — alphonse #3672 MERGED (new baseline val 70.34 / test 61.63); fern #3695 closed informative; R7 alphonse (#3817 FiLM ablation), fern (#3808 surf_weight), edward (#3786 Huber β) all assigned; 8/8 students staffed.
...
|----|---------|------------|--------|
| **#3817** | **alphonse** | **FiLM ablation under n_fourier=0 (2 arms: FiLM on vs off)** | **Just assigned R7 H27** |
...
| #3748 | nezuko | Spectral norm on output head + FiLM layers (3 arms) | WIP R6 H24 — running |
| #3712 | askeladd | Lion β1 sweep {0.8, 0.9, 0.95} | WIP R6 H23 — running |
| #3697 | frieren | Multi-σ Fourier {3,10,30} under FiLM+Lion+EMA | WIP R5 H20 — Arm C running |
| #3673 | tanjiro | EMA decay sweep {0.995, 0.997, 0.999} | Terminal posted (val 71.51, informative) → awaiting mark-ready |
| #3698 | thorfinn | TTA via z-reflection | Awaiting terminal SENPAI-RESULT → close informative-negative |

## Pending actions (next wakeup)

1. **Close tanjiro #3673** once marked ready (terminal posted: val 71.51, informative ablation) — then reassign tanjiro to R7
2. **Close thorfinn #3698** once terminal marker + mark-ready — then reassign thorfinn to R7
3. **Monitor frieren #3697** Arm C completion; evaluate and close/merge
4. **Monitor askeladd #3712** β1 sweep; close/merge when terminal posted
...
5. **Sobolev surface regularization points in right direction.** w=0.1 gives small test gain (−0.18) at flat val. Finer sweep reserved.
...
| Loss: Huber β | β ∈ {0.05 control, 0.1, 0.2} | #3786 edward |
...
For tanjiro/thorfinn (when closed) and further rounds:

1. **Multi-seed confirmation of n_fourier=0** — 3 seeds to verify Δ > σ≈4.6 (paper appendix table).
2. **Per-split loss weighting** — upweight camber_rc/in_dist at training time.
3. **Sobolev finer sweep** — w ∈ {0.03, 0.05, 0.08, 0.12, 0.15}.
4. **Multi-seed ensemble** — average predictions from 2–3 seeds of new baseline.
5. **Geometric features** — chord-normal coordinates, local surface curvature κ, surface-normal direction.
6. **Curriculum on Reynolds** — train low-Re samples first, ramp up. Targets re_rand OOD split.
7. **Wider model** — n_hidden 96→128 (if within wall-clock budget).
8. **Snapshot/EMA ensemble** — average EMA checkpoint predictions across multiple training points.
