...
|---|---|---|---|
| `surf-weight-50` (alphonse, #1431) | WIP-running | Raise surf_weight 10→50 (metric alignment); now arm `surf-weight-25-30m` mid-run, applied cruise-fix on own branch | Low–Med |
...
| `schedule-tuned-e13` (thorfinn, #1537) | WIP-finished-runs | --epochs 13; best finished `afft3f1v` at 122.25 (above ~99 mean baseline); new arm mid-run | Low |
...
| `re-resample` (askeladd, #1616) | WIP-finished-runs | log(Re)-weighted WeightedRandomSampler; uniform baseline arm `eztvtkxc` hit **90.91** (new W&B low); variant 100.61 (hurts) | Low–Med |
...
- **Schedule-budget mismatch is a structural baseline weakness.** With cosine T_max=50 but only ~14 epochs fitting in 30 min, the LR never anneals — meaning *all* in-flight Round 1 PRs are technically running on a stuck-at-peak-LR schedule. If frieren's `warmup-5ep` does well, the combo of warmup + tuned T_max is the obvious Round 2 stack. Note however that thorfinn's `schedule-tuned-e13` arm finished at 122.25 (worse than the ~99 mean of the combined-config baseline) — suggests just dropping `--epochs` is not enough; the LR-anneal-vs-converge trade-off needs warmup+T_max co-tuning.
