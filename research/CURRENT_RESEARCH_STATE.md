...
- **#3041 (frieren Re-jitter σ=0.10/0.30)** — σ=0.10: val=42.20 (+25.2% ❌), test=38.83 (+35.5% ❌); σ=0.30: val=38.11 (+13.1% ❌), test=34.24 (+19.5% ❌). All 4 splits regress on both arms including target test_re_rand (+36.1%/+14.3%). γ_w_L2 shrinks monotonically with σ (V-shaped depth pattern, ~15-25% smaller weights at σ=0.30 vs σ=0.10) — mechanism IS present (noise smooths γ-MLP surface) but smoothing costs more in fit than it gains in generalization. Counterintuitive σ ordering (σ=0.30 less bad than σ=0.10) confirms σ=0.10 is in a transitional regime. Meta-finding #28 added. Frieren reassigned to camber-weighted-loss (#3073).
...
28. **Conditioning-input-side OOD interventions fully retired under Lion + FiLM-Re (#3041 frieren Re-jitter, with #2960/#2984/#3034)** — Pre-γ-MLP-input Gaussian Re-jitter σ=0.10/0.30 regresses val by 13-25%, test by 19-36%; target test_re_rand regresses 14-36%. γ_w_L2 mechanism IS present (γ-MLP weight magnitudes shrink 15-25% under σ=0.30 vs σ=0.10, V-shaped depth pattern replaces baseline depth-monotone ramp) — noise reaches the γ-MLP and smooths its response surface as predicted. **But the smoothing trades off against capacity to encode the actual Re→γ mapping the model needs.** Counterintuitive σ ordering (σ=0.30 less bad than σ=0.10) shows σ=0.10 is in a transitional regime that disrupts fitting without producing coherent regularization. Combined with retired conditioning-Mixup (#2960, #2984 meta-finding #20) and Re-distribution rebalancing (#3034 meta-finding #23), **all THREE conditioning-input-side OOD interventions (jitter, mixup, rebalancing) are fully retired.** Future OOD interventions must operate on: (a) conditioning surface area (joint inputs: tanjiro #3067 camber-joint, askeladd #3072 AoA-block-0), (b) injection points (decoder #3028 MERGED, routing #3035 in-flight), or (c) gradient signal path NOT via conditioning (loss-side weighting #3073 just assigned — the first loss-side OOD intervention).
