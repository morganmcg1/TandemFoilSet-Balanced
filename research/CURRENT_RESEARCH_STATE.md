...
## Active Experiments (round 23:55 — channel-selective shape augmentation + multi-task aux output + structural-dichotomy probes)

| PR | Student | Slug | Status | Priority | Notes |
|----|---------|------|--------|----------|-------|
| #2650 | alphonse | `re-conditional-layernorm-affine` | WIP | **HIGH** | Re-conditional LN γ/β (shared, zero-init) — canonical Conditional-InstanceNorm pattern at the LN-affine injection point; bounded modulation by construction |
| #2671 | askeladd | `surface-only-normal-feature` | WIP (just-assigned) | **HIGH** | Zero normals on volume nodes; keep only at surface elements. Removes "ghost orientation" pollution from #2627. Clean test: if dichotomy persists, finding is fully structural (about trunk representation); if it disappears, the pollution side-channel was responsible. |
| #2599 | tanjiro | `se-channel-attention-r8` | WIP (pod rate-limit stuck) | MEDIUM | SE channel-attention after each block, zero-init residual gate. **Tanjiro pod still on secondary rate-limit cycle; PR labels correct but pod can't poll. Will recover naturally.** |
...
| #2660 | nezuko | `surface-normal-auxiliary-output-head` | WIP | **HIGH** | Auxiliary output head predicting (n_x, n_y) per surface element — Kendall/Gal/Cipolla 2017 multi-task. Forces trunk to develop normal-aware representations rather than reading normals from input channel. |
| #2662 | fern | `naca-jitter-ch16-17-only-sigma02` (corrected) | WIP (sent back after channel correction) | **HIGHEST** | **Channel-selective** NACA jitter: σ=0.02 on ch 16 (camber position) + ch 17 (thickness), **masking ch 15 (camber amplitude M = actual rc OOD axis per fern's empirical verification)**. Tests the Round 23:41 KEY FINDING with the CORRECTED channel index. |
| #2626 | thorfinn | `per-channel-separate-heads-kaiming-h128` | WIP (re-run) | **HIGH** | Re-issued with kaiming_normal init + head_hidden=128. Initial run was under-converged (zero-init) and showed cruise -44% / rc +21% dichotomy — 2nd of 3 structural dichotomy confirmations. |
