...
Active sub-axes:
1. **LR scan (tanjiro):** 7.5e-5→1e-4 (PR #2628) — continues the bisection upward
2. **Warmup (thorfinn):** 5-epoch linear warmup 0→7.5e-5 then cosine (PR #2631) — addresses 4-6× seed variance increase at 7.5e-5
3. **Weight decay (frieren):** wd=3e-3 at lr=7.5e-5 (PR #2629) — stronger L2 at higher LR
4. **Beta1 (edward):** beta1=0.95 vs default 0.9 (PR #2633) — reduces per-batch gradient noise in Lion's sign update
5. **Gradient Centralization (nezuko):** GC inside Lion step (PR #2564) — zero-mean gradient constraint before momentum update
6. **max_norm=0.5 (fern):** sent back for rebase onto new baseline (PR #2565) — small but reproducible win on old bar

**Independent axes still in flight:**
7. **SiLU activation (alphonse, PR #2505):** GELU→SiLU in FFN; needs rebase onto new baseline
8. **Per-channel Huber β_p=0.625 (askeladd, PR #2501):** upward β bisect for pressure channel; needs rebase
...
| #2516 | edward | Lion optimizer | **MERGED** 20:05 (val=50.19) |
| #2562 | tanjiro | Lion lr=7.5e-5 | **MERGED** 22:30 (val=45.43) — 10th baseline shift |
| #2561 | edward | Lion beta2=0.95 | **CLOSED** 22:50 (+14.8%, beta2 analogy wrong) |
| #2520 | thorfinn | n_head 4→8 | **CLOSED** 22:42 (+24% regression, capacity loss) |
| #2504 | frieren | QK-RMSNorm | **CLOSED** 22:43 (+14% regression, Q/K magnitude signal) |
| #2565 | fern | max_norm=0.5 | WIP — sent back for rebase onto new bar |
| #2564 | nezuko | Gradient Centralization | WIP — needs rebase onto lr=7.5e-5 |
| #2505 | alphonse | SiLU activation | WIP — needs rebase onto new baseline |
| #2501 | askeladd | β_p=0.625 | WIP — needs rebase onto new baseline |
| **#2628** | **tanjiro** | **Lion lr=1e-4** | **WIP NEW 22:55** |
| **#2631** | **thorfinn** | **Lion warmup 5ep** | **WIP NEW 22:55** |
| **#2629** | **frieren** | **Lion wd=3e-3** | **WIP NEW 22:55** |
| **#2633** | **edward** | **Lion beta1=0.95** | **WIP NEW 22:55** |

**Merged:** 10 | **Closed:** 31+ | **WIP:** 8 | **Idle:** 0
...
- **Cosine LR shape** (T_max, eta_min) — 3 negative results; T_max=50 implicit residual at epoch 35 is load-bearing
