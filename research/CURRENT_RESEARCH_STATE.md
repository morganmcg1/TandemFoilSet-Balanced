...
|----|---------|--------|--------|
| **#2470** | **alphonse** | **sw=15/sw=20 on n_head=1: test sw-reversal (sw<10 hurts, sw>10?)** | **WIP — new** |
...
- **#2416 (alphonse):** n_head=1+sw=5 interaction — sw=5 REGRESSES +3.94% (val=48.51 vs 46.67). sw signal reverses direction at n_head=1: synergistic at n_head=2, anti-synergistic at n_head=1. single_in_dist hardest hit (+4.26%). Mechanism: n_head=1 needs surface guidance; sw=5 strips it. Closed; reassigned to #2470 (sw=15/20 on n_head=1 — reversal test).
...
26. **sw signal REVERSES at n_head=1 (#2416):** sw=5 was synergistic at n_head=2 (−2.54 val, 1.75×); at n_head=1 sw=5 REGRESSES +1.84 val (+3.94%). Mechanism: n_head=1 (dim=128) must allocate head capacity to surface detail; sw=5 strips that signal with no backup head. Prediction: sw>10 should help on n_head=1 (reversal direction test → #2470 alphonse, sw=15/20).
