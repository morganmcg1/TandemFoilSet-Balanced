...
| 4 | **tanjiro** | **#3781** | **Re-conditioned per-sample loss reweighting (focal on Re tails, α=1.0)** | **WIP (assigned 07:22; fresh)** |
...
- **tanjiro Re-conditioned sample reweighting (#3781):** Per-sample loss weight `1 + α*(log_Re_z)²` concentrates gradient on Re-tail samples. Single arm α=1.0. Sanity check at α=0.0 mandatory. Zero compute overhead. Targets `test_single_in_dist` (78.65) and `test_re_rand` (65.00). Predicted conservative test ~65.5-66.5, hopeful ~64-65.
...
Key monitors:
- #3761 thorfinn slice_num {96,128}: predicted test ~62-64 if single_in_dist is partition-limited
- #3781 tanjiro Re-conditioned reweighting α=1.0: predicted test ~64-66 (single_in_dist + re_rand targets)
- #3746 fern clip-cap sweep: predicted 1-3% improvement over 66.87 if outlier-only mechanism confirmed
- #3693 alphonse LR sweep: predicted test ~64-68 (Arm A lr=1e-3 primary)
