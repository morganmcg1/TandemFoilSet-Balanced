...
## Active Experiments (round 00:45 — post-baseline-update, dichotomy-probe wave)

Baseline updated: **PR #2650 ReCondLN val_avg=28.2414**. All experiments below built on the new baseline.

| PR | Student | Slug | Status | Priority | Notes |
|----|---------|------|--------|----------|-------|
| #2678 | alphonse | `recondln-t-max35` | WIP | **HIGH** | T_max=35 (vs 28) on the new baseline — model was still descending at ep28 (best=last), this extends cosine warmdown under wall-cap. |
| #2671 | askeladd | `surface-only-normal-feature` | WIP | **HIGH** | Surface-only INPUT normal feature (zero volume-node normals). Pollution-isolation test #1 for the 6-way confirmed dichotomy. |
| #2688 | nezuko | `surface-only-aux-normal-head` | WIP (just-assigned) | **HIGH** | Surface-only AUX OUTPUT normal head — student's #1 follow-up to closed #2660. Mirror of askeladd's #2671 at the OUTPUT side. Together they isolate volume-pollution vs structural. |
| #2689 | frieren | `shape-bin-oversampling-m05` | WIP (just-assigned) | **HIGH** | **DATA-SIDE intervention**: oversample train samples with M≥0.5 by 3× via WeightedRandomSampler. Densifies train near rc OOD boundary on ch15. First non-feature/non-loss/non-architecture experiment. |
| #2690 | thorfinn | `re-conditional-output-bias` | WIP (just-assigned) | **HIGH** | 4th Re-conditioning hook — additive per-channel bias after ReScaleHead. Extends the only known winning axis (ReFiLM + ReScaleHead + ReCondLN) to a new bounded injection point (output bias). |
| #2662 | fern | `naca-jitter-ch16-17-only-sigma02` (corrected) | WIP (sent back after channel correction) | **HIGHEST** | **Channel-selective** NACA jitter: σ=0.02 on ch16+ch17, **masking ch15 (actual rc OOD axis)**. Tests the Round 23:41 KEY FINDING with the CORRECTED channel index. |
| #2599 | tanjiro | `se-channel-attention-r8` | WIP (pod rate-limit stuck) | MEDIUM | SE channel-attention; pod still on secondary rate-limit. |
