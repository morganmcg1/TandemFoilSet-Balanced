...
- **As of:** 2026-05-13 (updated cycle 68)
...
## Live PRs (active WIPs and recent closes — cycle 68)
...
| 2548 | askeladd | reflection-tta-inference | WIP (NEW cycle 67) | Krizhevsky 2012 / Shanmugam 2020. y-axis reflection TTA at inference, average pred(x) + pred(reflect_y(x)). Only pure inference-time ensemble in scope. NO training change. |
...
| 2498 | alphonse | tmult2-eta-min-compose | WIP (NEW cycle 62) | Compose T_0=7 T_mult=2 + eta_min=1e-5. Two orthogonal wins should stack. Preliminary: lnwuse3i val=81.82 (beats SOTA marginally), i42w5vvj val=85.35 (regresses) — 3.5 val seed spread is anomalously wide; investigating. |
| 2445 | nezuko | seed-variance-calibration | WIP (rebased + --seed committed at 19:17Z) | σ=0.34 val on #2227 config (mean 86.71 vs claim 83.997 — 8σ gap, code drift implied). Re-running on CURRENT SOTA #2444 config. |
...
44. **Reflection-symmetric Test-Time Augmentation (TTA)** — testing (#2548 askeladd, NEW cycle 67). Krizhevsky 2012 / Shanmugam 2020. Pure inference-time ensemble: forward pass on input + y-reflected input, average predictions (negating Uy component of reflected pred). NO training change. Mechanistically distinct from ALL training-time regularizers in scope — only purely inference-time ensemble being tested. Tests whether model has learned approximate symmetry-equivariance. Cheap turnaround (single training + 2 eval passes).
