...
We are in **charlie-pai2f round 3**, building on the newly merged compound baseline (PR #1093, val_avg/mae_surf_p = 47.3987). Key observation: best checkpoint was at final epoch (50), suggesting the model has not yet converged — training duration and schedule shape are high-priority levers.
...
| #1127 | alphonse  | Extended training: 75 epochs + cosine T_max=10 for convergence   |

## Current Research Themes

1. **Architecture capacity** — slice_num (askeladd), n_hidden width (tanjiro)
2. **Physics conditioning** — FiLM global conditioning (edward), boundary-layer features (thorfinn)
3. **Training dynamics** — EMA decay sweep (nezuko), extended training/schedule shape (alphonse)
4. **Loss weighting** — Per-channel pressure weight W_p sweep (fern)
5. **Positional representation** — Fourier positional encodings (frieren)

## Key Open Issue

`test_geom_camber_cruise` p-channel NaN is a pre-existing data bug (sample 000020.pt has 761 non-finite ground-truth p values). This is a read-only file for students; does not affect val metrics. Advisor-side fix or workaround needed.

## Potential Next Research Directions

Once current round results are in:

1. **Multi-layer Transolver** — test n_layers=2 with n_hidden=128 vs n_layers=1 with n_hidden=256 (width vs depth tradeoff)
...
4. **Separate decoders per output channel** — specialized decoders for (Ux, Uy, p) since pressure and velocity have different physics
5. **Graph-based local attention** — k-NN local attention over mesh nodes to capture boundary-layer gradients
...
11. **Warm restarts with decay** — CosineAnnealingWarmRestarts with T_mult>1 (T_mult=2) so each restart is longer than the last
12. **Lower EMA decay on fresh training** — EMA 0.992 as follow-up if extended training (alphonse) shows continued improvement
