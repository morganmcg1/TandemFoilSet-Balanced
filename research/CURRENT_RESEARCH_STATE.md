...
- **Last updated:** 2026-05-13 23:55 (Loop 27: #2604 fern hybrid_kendall_lr push CLOSED — both arms regress monotonically (lr=1e-3: val 46.93/test 39.82, lr=2e-3: val 46.94/test 39.84); **σ-spread ceiling found at 0.475**. 9 banked findings inc. **NEW channel-level falsification of compounding hypothesis** (single_in_dist test gained −2.11% from 0→0.475 but loses +1.04 to +2.35 from 0.475→0.82/0.92, U-shaped not monotonic), **NEW premature-commitment mechanism** (Kendall weight on surf_ux reaches ~120 vs baseline ~22), **NEW SWA rescue scales with lr** (1.6× more rescue at higher lr), 5th independent clip_fraction=1.0 confirmation (13+ total). Assigned #2666 fern β LOWER sweep {0.2, 0.15} — fern's own follow-up #2; tests β-side of β–σ coupling (different mechanism for spread vs head-lr push). All 8 students active. 5 PRs still stale_wip pending API rate-limit recovery.)
...
7. **film_mid_dim sweep {32, 128} on hybrid (#2616 alphonse)** — only untouched single-CLI capacity axis; targets the geom_camber_rc per-split asymmetry from #2500.
8. **huber_beta LOW sweep {0.2, 0.15} on hybrid (#2666 fern NEW)** — tests β-side of β–σ coupling; produces more spread via residual-shape mechanism rather than head-lr. **Diagnostic test:** distinguishes "premature commitment was the failure mode" from "U-curve is structural across mechanisms".
...
### 🔬 In-flight (Wave 12 — post-Loop-27)
...
- **film_mid_dim sweep {32, 128} on merged hybrid (#2616 alphonse)** — only untouched single-CLI capacity axis; targets geom_camber_rc per-split asymmetry from #2500
- **huber_beta LOW sweep {0.2, 0.15} on merged hybrid (#2666 fern NEW)** — β-side of β–σ coupling; distinguishes premature-commitment from structural U-curve
