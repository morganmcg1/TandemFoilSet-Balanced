...
- **Most recent direction from human researcher team:** None (no open issues at 02:50 UTC)
...
|----|---------|-----------|-------------|--------|
| **#4307** | **nezuko** | Lookahead α-bracket sweep (α=0.3, α=0.7) | NEW Lookahead+β2 baseline | WIP — NEW: characterizes Lookahead α; complement to alphonse's k-bracket |
| **#4309** | **askeladd** | n_head=4 architecture sweep | NEW Lookahead+β2 baseline | WIP — NEW: first architecture test on new optimizer stack |
| **#4311** | **fern** | Camber-stratified WeightedRandomSampler (3× oversample) | NEW Lookahead+β2 baseline | WIP — NEW: frequency-based (not loss-magnitude) camber targeting; different from #4204 |
| **#4313** | **frieren** | n_hidden=192 model capacity (+50%) | NEW Lookahead+β2 baseline | WIP — NEW: tests if model is capacity-limited post-optimizer optimization |
| **#4266** | **alphonse** | Lookahead k-bracket sweep (k=3 vs k=10) | NEW Lookahead+β2 baseline | WIP — k=5 from paper defaults, characterizing optimal k |
| **#4151** | **thorfinn** | LLRD=0.95 + Lookahead (gentler decay retest) | NEW Lookahead baseline | WIP — LLRD=0.85 was mixed (val −0.59%, test +0.88%); LLRD=0.95 may recover test regression |
| **#4251** | **edward** | Lookahead + lr=1e-3 | NEW Lookahead baseline | WIP — actively training (GPU 100%, 44GB); stale_wip flag was transient |
| **#4284** | **tanjiro** | weight_decay sweep (5e-4, 1e-3) | NEW Lookahead+β2 baseline | WIP — fresh axis; default 1e-4 never swept under Lookahead |
...
## Round 21 closures (this round)

| PR | Student | Hypothesis | val | Action |
|----|---------|-----------|-----|--------|
| ✓ **#4249** | **nezuko** | **Lookahead + β2=0.95 compound** | **52.944** | ✓ **MERGED — NEW BASELINE** |
| ✗ #4283 | askeladd | Lookahead+slice=16+β2=0.95 triple | 55.006 | ✗ Closed — FM1+FM2: substitutive on val_avg; β2=0.95 anti-compounds with slice=16 on camber_rc |
| ✗ #4267 | fern | AoA rotation aug ±5° | 56.069 | ✗ Closed — FM1 triggered (val>55); camber_rc barely −1%, cruise +17.77%; AoA aug axis fully closed |
| ✗ #4204 | frieren | Per-sample peak-\|p\| reweight α=1.0 | 64.396 | ✗ Closed — catastrophic: all splits regressed uniformly; backbone starvation from 19.7× weight dynamic range |
...
Priority order:
1. **#4307 nezuko α-bracket**: characterizes Lookahead axis with β2=0.95; small chance of finding better α
2. **#4311 fern camber-stratified sampler**: frequency-based camber targeting; different from failed reweighting
3. **#4313 frieren n_hidden=192**: model capacity test; optimizer may have eliminated optimization bottleneck
4. **#4309 askeladd n_head=4**: architecture axis; richer attention subspaces for camber geometry
5. **#4151 thorfinn LLRD=0.95+Lookahead**: gentler LLRD retest; could compound with new baseline
6. **#4266 alphonse k-bracket**: characterizes Lookahead k; complements α-bracket
7. **#4284 tanjiro weight_decay**: fresh optimizer-regularization axis
...
- **GPU utilization**: 100% — all 8 students assigned as of 02:50 UTC
