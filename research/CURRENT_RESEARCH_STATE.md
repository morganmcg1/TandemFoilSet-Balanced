...
- 2026-05-13 15:00 — willow-pai2g-48h-r1, round 3 ongoing. **CURRENT BEST: test=61.8457 (PR #2282 slice_num=24 + clip=5.0)**. Cumulative gain from PR #1391: 121.28 → 61.85 = **−49.0%**.
...
Config: bf16 + bs=4 + accum=2 + Lion lr=1.5e-4 + β1=0.9 + wd=1e-4 + Fourier L=8 + n_hidden=192, n_layers=5, n_head=4, **slice_num=24**, mlp_ratio=2 + **grad_clip_max_norm=5.0** + surf_weight=10 + act=gelu + eta_min=0 + dropout=0. W&B run: evcflzgo.
...
## Round-3 status (updated 2026-05-13 15:00)
...
| tanjiro | #2344 | attention dropout=0.1 | **wip** (NEW) |
| thorfinn | #2343 | weight decay ablation (wd=0) | **wip** (NEW) |
...
4. **Does weight decay impede Lion+slice regularization?** (NEW)
5. **Does attention dropout help rc/re_rand OOD?** (NEW)
6. **Does eta_min refinement tail help?** (alphonse #2326)
7. **Does SiLU outperform GELU?** (edward #2327)
8. **What's optimal Lion β1 with clip?** (askeladd #2237)

## Plateau watch
NOT in plateau. FOUR consecutive wins (slice scan). Capacity-budget bound fully formalized. Clip mechanism fully characterized. Warmup lever closed (per-epoch bug; would need per-iteration reimplementation). All 8 students have active per-epoch-cost-neutral experiments. Continue mining.
