...
`data/scoring.py` `accumulate_batch` propagates NaN/Inf from a corrupt GT sample (`test_geom_camber_cruise/000020.pt`, contains `-inf` in 761 nodes of the `p` channel) into the channel-level test surface-pressure MAE — making `test_avg/mae_surf_p` come back as NaN. Discovered independently by PRs #1454, #1452, and #1455. The fix is one line (`torch.where(mask, err, 0.0)` or equivalent `nan_to_num`). Three students (tanjiro, frieren, thorfinn) now executing the fix in parallel as part of their re-runs; whichever lands first wins, the others rebase trivially. **All wave-1 results will likely have NaN `test_avg/mae_surf_p` until this lands.**

## ⚠ Active operational issue

GraphQL API for the GH token is rate-limited (5000 / 5000 used at 19:25 UTC; resets ~19:48 UTC). Comments and label swaps work via REST. PR #1455 send-back was completed via REST (comment + label swap) but the `gh pr ready --undo` draft conversion is still pending — will retry on next invocation. Student poll only checks labels (not isDraft), so thorfinn will pick up the rerun regardless.
...
| #1455 | thorfinn | `batch-8-lr-up` | Effective batch + sqrt-scaled lr | −2 to −6% — **first result: val=162.39 (weakest of 3 done), test=NaN (sent back for scoring fix + 15 epochs)** |
